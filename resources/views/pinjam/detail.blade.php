                        ...
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                          ...
                          <tbody>
                              ...
                              <tr>
                                ...
                                {{-- <td></td> --}}
                                <td>
                                  @if (isEmpty($tran->waktu_kembali))
                                  <form action="{{route('NotifyEmail')}}">
                                    <input type="hidden" name="id_peminjaman" value="{{$tran->id_peminjaman}}">
                                      <input type="hidden" name="id_peminjam" value="{{$tran->peminjam_id}}">
                                      <button type="submit">Kirim Email Pengingat</button>
                                    </form>
                                    @else
                                    {{$tran->waktu_kembali}}
                                    @endif
